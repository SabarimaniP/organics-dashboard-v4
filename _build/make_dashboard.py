...
.bkgrid{display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(238px,1fr))}
...
.bkrow{display:grid;grid-template-columns:64px 1fr 52px 46px;gap:7px;align-items:center;
...
.mini{background:var(--chip);border-radius:2px;height:11px}
...
<nav class="tabs" id="tabs">
  ...
  <button aria-selected="false" data-t="find">Top Findings</button>
</nav>
...
<section class="panelbox" id="p-find">
  <div class="panel">
    <div class="fhead">July cohort &nbsp;·&nbsp; <b>19,096</b> leads created &nbsp;→&nbsp;
      <b>12,579</b> assigned &nbsp;→&nbsp; <b>10,071</b> called &nbsp;→&nbsp;
      <b>7,158</b> answered &nbsp;→&nbsp; <b>385</b> demos booked</div>

    <ol class="findlist">
      <li><span class="ftext"><b>Assignment is the biggest gap — only 65.9% of leads reached a salesperson.</b>
        <span class="fsub">6,517 of 19,096 July leads were never assigned. June was 83.7%, so this got worse.</span></span></li>

      <li><span class="ftext"><b>Two-thirds of that gap is one source: Learn App is only 51% assigned.</b>
        <span class="fsub">4,303 Learn App leads unassigned. It converts at 3.64% of assigned — as good as IL Website.</span></span></li>

      <li><span class="ftext"><b>2,508 assigned leads were never called at all.</b>
        <span class="fsub">Median age 21 days, and 1,265 of them are grade 11–12 — our weakest grade — so expect less than average.</span></span></li>

      <li><span class="ftext"><b>Consumption jumped 38.9% → 80.1% of assigned leads called. That is what doubled July.</b>
        <span class="fsub">Demos went 228 (June) → 385 (July) with no change in lead quality. Hold this rate in August.</span></span></li>

      <li><span class="ftext"><b>Grade 6–10 converts 3.6× better than 11–12.</b>
        <span class="fsub">6–10: 4.31% of assigned book a demo. 11–12: 1.20%. Put the calling hours on 6–10 first.</span></span></li>

      <li><span class="ftext"><b>Answered → demo slipped 7.42% (June) to 5.38% (July).</b>
        <span class="fsub">We reached far more people and a smaller share said yes. Volume hid it. Restoring June's rate is worth +146.</span></span></li>
...
function breakupControls(hostId,fi){
  ...
  [['c','Cohort'],['r','Rolling'],['b','Both']].forEach(([v,t])=>{
  ...
  $('#'+elId).innerHTML=parts.map(c=>{
    ...
    return '<div class="bkcard"><h3>'+esc(c.name)
      ...
      +STAGES.map(([k,l],i)=>{
        ...
        let cell='\u2014', cls='';
        if(was!==null){
          if(was===0&&c.f[k]===0){cell='\u2014';}
          else if(was===0){cell='new'; cls='up';}
          else {const ch=100*(c.f[k]-was)/was;
            cell=(ch>=0?'+':'\u2212')+Math.abs(ch).toFixed(0)+'%'; cls=ch>=0?'up':'down';}
        }
        return '<div class="bkrow" data-tip="<b>'+esc(c.name)+' \u00b7 '+l+'</b><br>'+fmt(c.f[k])
          ...
          +'<span class="p '+cls+'">'+cell+'</span></div>';}).join('')
      +'</div>';}).join('');
}
...
function weekly(){
  ...
  h+=`<tr class="tot"><td class="lft">All 13 weeks</td><td>${fmt(TL)}</td><td>${fmt(TA)}</td>`
...
function compare(){
  ...
  $('#cmpbody').innerHTML=`<div class="tblwrap"><table><thead><tr><th class="lft">&nbsp;</th>
   ...
   +STAGES.map(([k,l])=>{const d=oa[k]-ob[k];
     return `<tr><td class="lft" style="font-weight:600">${l}</td><td>${fmt(oa[k])}</td><td>${fmt(ob[k])}</td>
