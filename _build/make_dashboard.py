...
/* min card width must leave room for the bar: the 5 fixed bkrow columns + gaps + padding come to
   ~238px, so anything near that collapses the 1fr bar to nothing. 292px keeps it legible. */
.bkgrid{display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(292px,1fr))}
...
/* 5 columns: label | bar | count | step % (vs the stage above) | change vs the previous period */
.bkrow{display:grid;grid-template-columns:60px 1fr 48px 42px 44px;gap:6px;align-items:center;
...
.bkhead{display:grid;grid-template-columns:60px 1fr 48px 42px 44px;gap:6px;padding:4px 10px 3px;
...
.bkhead span:nth-child(3),.bkhead span:nth-child(4),.bkhead span:nth-child(5){text-align:right}
...
function breakup(elId,fi){
  ...
  $('#'+elId).innerHTML=parts.map(c=>{
    ...
    return '<div class="bkcard"><h3>'+esc(c.name)
      ...
      +'<div class="bkhead"><span>Stage</span><span></span><span>Count</span><span>Step</span>'
      +'<span>'+(plab?'vs prev':'')+'</span></div>'
      +STAGES.map(([k,l],i)=>{
        ...
        return '<div class="bkrow" data-tip="<b>'+esc(c.name)+' \u00b7 '+l+'</b><br>'+fmt(c.f[k])
          ...
          +'<span class="s">'+(st===null?'—':p1(st))+'</span>'
